...
namespace Stateless
{
    public partial class StateMachine<TState, TTrigger>
    {
        ...
        public class StateConfiguration
        {
			...
			public StateConfiguration OnUpdate(UnityEngine.MonoBehaviour script, string func, float rate) {
				this
					.OnEntry(() => script.InvokeRepeating(func, 0.01f, rate))
            ...
            private class CoroClass {
				...
				public System.Collections.IEnumerator CoroRunner() {
					yield return null;
					while (!exit) {
						action();
						yield return new UnityEngine.WaitForSeconds(rate);
					}
				}
			}

			public StateConfiguration OnUpdate(UnityEngine.MonoBehaviour script, Action func, float rate) {
				var coro = new CoroClass { action = func, rate = rate };
				this
					.OnEntry(() => script.StartCoroutine(coro.CoroRunner()))
					.OnExit(() => coro.exit = true);
